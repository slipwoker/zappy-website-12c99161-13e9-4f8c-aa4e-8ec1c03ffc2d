document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      if (value !== '') {
        ...
        if ((type === 'text' || tagName === 'textarea') && field.minLength > 0 && value.length < field.minLength) {
          showError(field, label + ' must be at least ' + field.minLength + ' characters.');
          return false;
        }

        if (type === 'text' && field.maxLength > 0 && value.length > field.maxLength) {
          showError(field, label + ' must not exceed ' + field.maxLength + ' characters.');
          return false;
        }

        if (type === 'checkbox' && isRequired && !field.checked) {
          showError(field, label + ' must be checked.');
          return false;
        }
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      const fields = contactForm.querySelectorAll('input:not([type="hidden"]):not([type="submit"]):not([type="reset"]):not([type="button"]), textarea, select');
  ...
  // ============================================================
  const animatedElements = document.querySelectorAll('.fade-in, .slide-up, .slide-in, [data-animate]');
  ...
  if (animatedElements.length > 0) {
    ...
    function applyInitialStyles(el) {
      const animationType = el.dataset.animate || (el.classList.contains('slide-up') ? 'slide-up' : el.classList.contains('slide-in') ? 'slide-in' : 'fade-in');
    ...
    if ('IntersectionObserver' in window) {
      ...
      animatedElements.forEach(applyInitialStyles);
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "תיבת הדמיון משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '12c99161-13e9-4f8c-aa4e-8ec1c03ffc2d',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '12c99161-13e9-4f8c-aa4e-8ec1c03ffc2d';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '12c99161-13e9-4f8c-aa4e-8ec1c03ffc2d';
